...
/*????????????????????????????????????*/
    ...
    * It is assumed that tasks do not have entries in fproc/mproc.*/
    int pstat(int p_nr, struct pstat *bufp, int endpoints){
        ...
        if (isemptyp(&ps_proc[p_ki]) && !(ps_mproc[p_nr].mp_flags & IN_USE)) {        
                return-1;        
            }
        ...
        if (p_nr >= 0) {
            ...
            if(endpoints) bufp->ps_pid = ps_proc[p_ki].
            p_endpoint;
            else bufp->ps_pid = ps_mproc[p_nr].mp_pid;
            bufp->ps_ppid = ps_mproc[ps_mproc[p_nr].mp_parent].
            mp_pid;
    ...
    bufp->ps_tsize = (size_t) ps_proc[p_ki].p_memmap[T].
    mem_len << CLICK_SHIFT;
    bufp->ps_dsize = (size_t) ps_proc[p_ki].p_memmap[D].
    mem_len << CLICK_SHIFT;
    bufp->ps_ssize = (size_t) ps_proc[p_ki].p_memmap[S].
    mem_len << CLICK_SHIFT;
    bufp->ps_vtext = (off_t) ps_proc[p_ki].p_memmap[T].
    mem_vir << CLICK_SHIFT;
    bufp->ps_vdata = (off_t) ps_proc[p_ki].p_memmap[D].
    mem_vir << CLICK_SHIFT;
    bufp->ps_vstack = (off_t) ps_proc[p_ki].p_memmap[S].
    mem_vir << CLICK_SHIFT;

    bufp->ps_text = (off_t) ps_proc[p_ki].p_memmap[T].
    mem_phys << CLICK_SHIFT;
    bufp->ps_data = (off_t) ps_proc[p_ki].p_memmap[D].
    mem_phys << CLICK_SHIFT;
    bufp->ps_stack = (off_t) ps_proc[p_ki].p_memmap[S].
    mem_phys << CLICK_SHIFT;
